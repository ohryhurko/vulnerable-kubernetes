#!/bin/bash
# Vulnerable Kubernetes Cluster Setup Script
# Deploys intentionally vulnerable applications and configurations
...
echo 'Successfully deployed vulnerable cluster. Ready for security assessment!'
echo 'Ensure pods are in running status before running access-cluster.sh script'
echo 'Now run the bash access-cluster.sh to access the vulnerable environment.'
